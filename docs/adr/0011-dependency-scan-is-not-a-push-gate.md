...
**버전을 13.0.0 으로 올리고, GitHub Actions 에서는 푸시 게이트에서 빼낸다.**

- `secrets` job (gitleaks): 모든 push / PR 에서 돈다. 빠르고 결정적이며 외부 의존이 없다.
- `dependency-scan` job (OWASP): `workflow_dispatch` 에서만 돈다.
...
지금은 `workflow_dispatch` 전용이다.

이 문서는 그 대가를 감추지 않는다. 위의 "근거" 절은 이 검사가 **시간에 걸리는 검사**라고
적었는데, 주기 실행을 빼면 그 시간축이 사라진다. 코드를 한 줄도 건드리지 않은 저장소에서
새 CVE 가 공개되면, **누군가 손으로 Run workflow 를 누르기 전까지 아무도 모른다.**
운영 중인 서비스라면 받아들일 수 없는 상태다.

포트폴리오 저장소라는 맥락에서는 성립한다. 배포된 인스턴스가 없고, 스캔은 면접에서
"이런 게이트를 둘 줄 안다"를 보이기 위한 것이며, 필요할 때 눌러서 돌리면 된다.
**운영으로 옮긴다면 주기 실행부터 되살려야 한다.** 아래 "실운영이라면 추가할 것"에 넣는다.

## 결과

- 푸시/PR CI 는 외부 API 가용성과 무관해진다.
- NVD 스캔은 Actions 탭에서 **Run workflow** 로만 돈다. 자동으로는 영원히 돌지 않는다.
